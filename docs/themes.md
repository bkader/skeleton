...
* Themes are translatable. By simply setting where language files are located and adding language lines, you can use your translations anywhere in your views. You can even override existing language lines.
...
Once that done, use CodeIgniter language helper or provided functions: **line**, **__** or **\_e**. More to came later.
...
* **html_class**: this filter is used to set classes to the &lt;html&gt; tag ([example #1](#example-1)).
* **language_attributes**: this filter is used to add anything to the **lang** attributes of the &lt;html&gt; tag ([example #2](#example-2)).
* **the_charset**: this filter alters the &lt;meta charset=""&gt; tag the way you want ([example #3](#example-3)).
* **the_title**: If you want to alter the page title, you may use this hook ([example #4](#example-4)).
* **extra_head**: this filter is used to add anything right before the closing &lt;/head&gt; tag ([example #5](#example-5)).
* **body_class**: this filter is used to set classes to the &lt;body&gt; tag ([example #6](#example-6)).
* **the_content**: this filter is used to alter the final view content, it takes a string as argument ([example #7](#example-7)).
* **theme_images**: this action is used to define default images thumbnails sizes generated by the media manager ([example #8](#example-8)).
